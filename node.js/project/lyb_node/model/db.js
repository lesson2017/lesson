...
exports.find = function (collectionsName,json,args,callback) {
    ...
    connectDB(function (err,db) {
        ...
        var cursor = db.collection(collectionsName).find(json).skip(skipNum).limit(limitNum).sort(sort);
...
//获取总共有多少条数据
exports.getAllCount = function (collectionsName,callback)
{
    connectDB(function (err,db) {
       db.collection(collectionsName).count({}).then(function (count) {
           callback(count);
           db.close();
       });
    })
